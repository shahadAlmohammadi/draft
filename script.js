...
window.onload = () => {
  ...
  recognition.onresult = (event) => {
    ...
    if (matchedQA) {
      ...
      notFoundAudio.play().catch((err) => {
        logToUI("❌ Not found audio failed:", err);
  ...
  recognition.onerror = (event) => {
    ...
    errorAudio.play().catch((err) => {
      logToUI("❌ Could not play fallback audio:", err);
  ...
  recognition.onend = () => {
    ...
    if (isRecognitionRunning) {
      ...
      setTimeout(() => {
        try {
          recognition.start();
          logToUI("Start recognition");
          console.log("Start recognition");
        } catch (e) {
          logToUI("❌ Could not restart:", e);
